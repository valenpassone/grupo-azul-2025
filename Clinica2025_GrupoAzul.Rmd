...
### **Preparación de dataframes y procesamiento de datos Censos 2010  y 2022**
...
*Se unieron cada una de las bases correspondientes por año de toma (2010 y 2022):*
...
*Se construyeron las variables de déficit habitacional:*
...
*Se asigno el código de provincia a las bases por departamento:*
...
*Se generó una nueva base agrupando por provincias y obtiendo variables resumen para ellas:*
...
*Se calculó la media nacional de déficit cuantitativo compuesto y cualitativo para 2010:*

```{r}
mean(censo2010_prov_group$defhab_compuesto_pct_10)
```

```{r}
mean(censo2010_prov_group$def_cuali_pct_10)
```
*Se repitieron los mismos procesos para la base de 2022:*
...
*Media nacional déficit cuantitativo compuesto 2022:*

```{r}
mean(censo2022_prov_group$defhab_compuesto_pct_22)
```

*Media nacional déficit cualitativo 2022:*

```{r}
mean(censo2022_prov_group$def_cuali_pct_22)
```

*Se unieron las bases y se generó una nueva variable que identifique la variación intercensal:*
...
*Se calcularon las medias de las variaciones intercensales:*

```{r}
mean(censo2010_2022_prov$var_def_compuesto)
```

```{r}
mean(censo2010_2022_prov$var_def_cuali)
```

*Para realizar mapas se bajaron las bases georeferenciadas por provincia y por departamento. Se realizaron los cambios necesarios para poder matchear:*
...
*Se agregaron los datos georreferenciados a las bases:*
...
*Finalmente se guardaron las bases para su procesamiento en QGIS:*
...
### **Gráficos Censos 2010 y 2022 - País**

*Se realizaron las adecuaciones necesarias para el gráfico deseado:*
...
**Gráfico variación déficit cuantitativo:**
...
**Gráfico variación déficit cualitativo y cuantitativo por provincia:**

```{r}
df_variacion_graf <- censo2010_2022_prov %>%
  select(Provincia, var_def_compuesto, var_def_cuali) %>%
  pivot_longer(cols = c(var_def_compuesto, var_def_cuali),
    names_to = "Tipo_Variacion",
    values_to = "Valor") %>%
  mutate(Tipo_Variacion = case_when(Tipo_Variacion == "var_def_compuesto" ~ "Var. Déficit Cuantitativo Compuesto", Tipo_Variacion == "var_def_cuali" ~ "Var. Déficit Cualitativo", TRUE ~ Tipo_Variacion),Tipo_Variacion = factor(Tipo_Variacion, levels = c("Var. Déficit Cuantitativo Compuesto","Var. Déficit Cualitativo")))

ggplot(df_variacion_graf, aes(x = Tipo_Variacion, y = Valor, fill = Tipo_Variacion)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8)) +
  facet_wrap(~ Provincia, ncol = 6) +
  labs(title = "Variación del Déficit (%) por Provincia: Cuantitativo vs Cualitativo (2010-2022)",
...
tabla_bajos_compuesto <- df_tabla_bajos %>%
  gt() %>%
  tab_header(title = "Provincias con mayor variación de déficit habitacional cuantitativo",subtitle = "Período 2010-2022") %>%
    cols_label(Provincia = "Provincia",
    var_def_compuesto = "Variación (%)") %>%
    fmt_percent(columns = var_def_compuesto,
    scale_values = FALSE,
    decimals = 2) %>%
...
tabla_altos_cuali <- df_tabla_altos %>%
  ...
  fmt_percent(columns = var_def_cuali,
    scale_values = FALSE,
    decimals = 2) %>%
  cols_align(align = "center",
...
### **Análisis de caso: Santiago del Estero**

*Con la función filter () se seleccionaron únicamente los departamentos de Santiago del Estero*
...
```{r}
mean(santiago_2010_2022$defhab_compuesto_pct_10)
```
```{r}
mean(santiago_2010_2022$defhab_compuesto_pct_22)
```
```{r}
mean(santiago_2010_2022$def_cuali_pct_10, na.rm = TRUE)
```
```{r}
mean(santiago_2010_2022$defhab_compuesto_pct_22)
```

*Se crearon las variables para calcular la variación intercensal:*
...
```{r}
mean(santiago_2010_2022$var_def_compuesto)
```
```{r}
mean(santiago_2010_2022$var_def_cuali, na.rm =TRUE)
```
**Gráfico Evolución de población, hogares y viviendas:**
...
df_lineas_sde <- censo2010_2022_prov %>%
  ...
  pivot_longer(cols = -Provincia,
    names_to = "col_temp",
    values_to = "Valor") %>%
  mutate(Año = if_else(str_detect(col_temp, "2010$"), "2010", "2022"),
...
**Gráfico Crecimiento relativo de variables:**

```{r}
base_2010 <- df_lineas_sde %>%
  filter(Año == "2010") %>%
  select(Tipo, Valor_Base = Valor)
df_indexado <- df_lineas_sde %>%
  left_join(base_2010, by = "Tipo") %>%
  mutate(Indice = (Valor / Valor_Base) * 100)

ggplot(df_indexado, aes(x = Año, y = Indice, color = Tipo, group = Tipo)) +
...
*Descarga de bases para el uso de QGIS:*
